...
def load_documents_main():
    ...
    clean_json_path = Path(DOCUMENTS_DIR) / "NC5_clean.json"
    if clean_csv_path.exists():
        print(f"Conversion et chargement du CSV : {clean_csv_path.name}")
        csv_to_json(str(clean_csv_path), str(clean_json_path))
        json_docs = load_json_for_rag(str(clean_json_path))
        if json_docs: all_documents.extend(json_docs)
        print(f"Chargé {len(json_docs)} documents depuis le JSON (issu du CSV, 1 doc par ligne).")
    else:
